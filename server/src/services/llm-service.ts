...
export class LLMService {
  ...
  async transform(request: TransformRequest): Promise<TransformResponse> {
    ...
    switch (request.target) {
      ...
      case 'vertex':
        return this.vertexService.transform(request, this.buildSystemPrompt(request.operation, request.text))
      case 'openai':
  ...
  private async transformWithLMStudio(request: TransformRequest): Promise<TransformResponse> {
    const systemPrompt = this.buildSystemPrompt(request.operation, request.text)
  ...
  private buildSystemPrompt(operation: string, requestText: string): string {
    switch (operation) {
      ...
      case 'variation-slight':
        return `Create a slightly different variation of the passed text. Keep the same length (approximately ${requestText.length} characters), meaning, and core message, but rephrase it with minor word changes and slight structural adjustments. Return only the transformed text without any preamble or explanation. The result text should be the same type of text as the input text.`

      case 'variation-fair':
        return `Create a fairly different variation of the passed text. Keep a similar length (approximately ${requestText.length} characters) and the same general meaning, but use different vocabulary, sentence structures, and phrasing approaches while maintaining the core concept. Return only the transformed text without any preamble or explanation. The result text should be the same type of text as the input text.`

      case 'variation-very':
        return `Create a very different variation of the passed text. Keep a similar length (approximately ${requestText.length} characters) and the same general theme or subject matter, but express it in a significantly different way with different vocabulary, structure, perspective, or framing. Be creative while still addressing the same underlying topic. Return only the transformed text without any preamble or explanation. The result text should be the same type of text as the input text.`

      case 'explore':
        return `Generate exactly 5 variations of the passed text. Each variation should be approximately ${requestText.length} characters in length and should be the same type of text as the input. The variations should be progressively more different:
...
5. [fifth variation]`
      ...
      case 'generate':
        return `The user will provide a concept or idea. Generate exactly 5 different, descriptive suggestions based on that concept. Each suggestion should be a specific example or instance of the concept provided.
...
5. [fifth suggestion]`
      ...
      case 'generate-wildcard':
        return `The user will provide a concept or category. Generate exactly 20 different values that would be appropriate for interpolation as wildcard values in that category. Each value should be a specific, concrete example or instance.
...
20. [twentieth value]`
      ...
      case 'auto-label':
        return `The user will provide some text content. Analyze the text and generate both a descriptive title and a code-friendly identifier.
