...
export function WildcardBrowserLists({ wildcard, currentPath, onSelectValue }: WildcardBrowserListsProps) {
  ...
  const buildTree = useMemo(() => {
    ...
    const buildTreeRecursive = (obj: unknown, parentPath: string[] = []): TreeNode[] => {
  ...
  const handleNavigate = (key: string, isLeaf: boolean, path: string, value: unknown, levelIndex: number) => {
    if (isLeaf) {
                ...
                return (
                  ...
                  >
                    <div className="text-left">
                      <div className="font-medium">{node.isLeaf ? String(node.value) : node.key}</div>
                    </div>
                  </button>
                )
              })}
