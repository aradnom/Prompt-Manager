...
export class VertexService {
    ...
    if (this.config.vertex.projectId) {
      ...
      if (this.config.vertex.serviceAccountJson) {
        const resolvedPath = path.resolve(
          process.cwd(),
          this.config.vertex.serviceAccountJson,
        );
  ...
  async transform(
    ...
    try {
      console.debug(
        `Attempting to generate content with model: ${this.config.vertex.model} in project: ${this.config.vertex.projectId}, location: ${this.config.vertex.location}`,
      );
