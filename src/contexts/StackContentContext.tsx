...
const StackContentContext = createContext<StackContentContextType | undefined>(undefined)

export function StackContentProvider({ children }: { children: ReactNode }) {
  const [renderedContent, setRenderedContent] = useState('')
  const [renderedContentWithMarkers, setRenderedContentWithMarkers] = useState('')

  return (
    <StackContentContext.Provider value={{ 
      renderedContent, 
      setRenderedContent,
...
export function useStackContent() {
  ...
  if (context === undefined) {
    throw new Error('useStackContent must be used within a StackContentProvider')
  }
  return context
}
