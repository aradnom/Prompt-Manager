...
interface TextWithWildcardsProps {
  ...
  onWildcardPathChange?: (displayId: string, oldPath: string, newPath: string) => void
}

export function TextWithWildcards({ text, className, valueOnly = false, enableTooltips = false, onWildcardPathChange }: TextWithWildcardsProps) {
  const { data: wildcards } = api.wildcards.list.useQuery()

  const elements = useMemo(() => {
    // 1. Parse standard wildcards
    const wildcardMatches = parseWildcards(text).map(m => ({ ...m, type: 'wildcard' as const }))
    ...
    const allMatches = [...wildcardMatches, ...valueMatches].sort((a, b) => a.index - b.index)
