...
const ActiveStackContext = createContext<ActiveStackContextType | undefined>(
  undefined,
);
...
export function ActiveStackProvider({ children }: { children: ReactNode }) {
  ...
  const [activeStackBlocks, setActiveStackBlocks] = useState<
    BlockWithRevisions[]
  >([]);
...
export function useActiveStack() {
  ...
  if (context === undefined) {
    throw new Error(
      "useActiveStack must be used within an ActiveStackProvider",
    );
  }
  return context;
}
