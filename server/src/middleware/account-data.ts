 ...
 */
export function withDerivedKey(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!req.session.encryptedDerivedKey) {
      return res.status(401).json({ error: 'Session key not found. Please log in again.' })
    }

    // Get session key from cookie
    const sessionKey = req.cookies.sessionKey
    if (!sessionKey) {
      return res.status(401).json({ error: 'Session key cookie missing. Please log in again.' })
    }

    // Decrypt the derived key
    const derivedKey = decryptDerivedKey(req.session.encryptedDerivedKey, sessionKey)
