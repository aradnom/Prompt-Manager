  ...
  if (Array.isArray(obj)) {
    obj.forEach((item, index) => {
      if (typeof item === 'string') {
        ...
        values.push(...extractValuesFromObject(item, [...currentPath, index], depth + 1))
      } else {
    ...
    Object.keys(record).forEach(key => {
      ...
      if (typeof value === 'string') {
        ...
        values.push(...extractValuesFromObject(value, [...currentPath, key], depth + 1))
      } else {
...
export function extractWildcardValues(content: string, format: string): WildcardValue[] {
  try {
    switch (format) {
      case 'json': {
...
export function resolveWildcardPath(content: string, format: string, path: string): string | null {
  try {
    if (!path) {
